...
int initialise(const char* paramfile, const char* obstaclefile,
               t_param* params, float** speeds_ptr, float** tmp_speeds_ptr,
...
int grid_ops(const t_param params, t_speed**restrict cells, t_speed**restrict tmp_cells, const int* obstacles);
...
int finalise(const t_param* params, float** speeds_ptr, float** tmp_speeds_ptr,
             int** obstacles_ptr, float** av_vels_ptr);
...
int main(int argc, char* argv[])
{
  ...
  init_tic=tot_tic;
  initialise(paramfile, obstaclefile, &params, &(cells.speeds), &(tmp_cells.speeds), &obstacles, &av_vels);
  ...
  finalise(&params, &(cells.speeds), &(tmp_cells.speeds), &obstacles, &av_vels);
...
int grid_ops(const t_param params, t_speed**restrict cells_ptr, t_speed**restrict tmp_cells_ptr, const int*restrict obstacles)
{
  t_speed*restrict cells = *cells_ptr;
  t_speed*restrict tmp_cells = *tmp_cells_ptr;
    ...
    for (int ii = 0; ii < params.nx; ii++)
    {
      ...
      ** scratch space grid */
      ...
      if (!obstacles[ii + jj*params.nx])
      {
        ...
        const float u[] = {0, u_x, u_y, -u_x, -u_y, u_x + u_y, -u_x + u_y, -u_x - u_y, u_x - u_y};
        //u[1] =   u_x;        /* east */
        //u[2] =         u_y;  /* north */
        //u[3] = - u_x;        /* west */
        //u[4] =       - u_y;  /* south */
        ...
        const float d_equ[] = {w0 * local_density * (1.f - u_sq / (2.f * c_sq)),
          w1 * local_density * (1.f + u[1] / c_sq + (u[1] * u[1]) / (2.f * c_sq * c_sq) - u_sq / (2.f * c_sq)),
          w1 * local_density * (1.f + u[2] / c_sq + (u[2] * u[2]) / (2.f * c_sq * c_sq) - u_sq / (2.f * c_sq)),
          w1 * local_density * (1.f + u[3] / c_sq + (u[3] * u[3]) / (2.f * c_sq * c_sq) - u_sq / (2.f * c_sq)),
          w1 * local_density * (1.f + u[4] / c_sq + (u[4] * u[4]) / (2.f * c_sq * c_sq) - u_sq / (2.f * c_sq)),
          w2 * local_density * (1.f + u[5] / c_sq + (u[5] * u[5]) / (2.f * c_sq * c_sq) - u_sq / (2.f * c_sq)),
          w2 * local_density * (1.f + u[6] / c_sq + (u[6] * u[6]) / (2.f * c_sq * c_sq) - u_sq / (2.f * c_sq)),
          w2 * local_density * (1.f + u[7] / c_sq + (u[7] * u[7]) / (2.f * c_sq * c_sq) - u_sq / (2.f * c_sq)),
          w2 * local_density * (1.f + u[8] / c_sq + (u[8] * u[8]) / (2.f * c_sq * c_sq) - u_sq / (2.f * c_sq))};
               ...
               int** obstacles_ptr, float** av_vels_ptr)
{
  ...
  */
  ...
  /* main grid */
  *speeds_ptr = (float*)malloc(sizeof(float) * (params->ny * params->nx));

  if (*speeds_ptr == NULL) die("cannot allocate memory for cells", __LINE__, __FILE__);

  /* 'helper' grid, used as scratch space */
  *tmp_speeds_ptr = (float*)malloc(sizeof(float) * (params->ny * params->nx));

  if (*tmp_speeds_ptr == NULL) die("cannot allocate memory for tmp_cells", __LINE__, __FILE__);
